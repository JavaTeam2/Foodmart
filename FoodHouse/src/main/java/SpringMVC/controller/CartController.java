		...
		System.out.println("cartForm: " + cartForm.getId());
	       ...
	       if (food != null) {
 			   ...
 			   for(OrderDetail orderDetail: cartForm.getListUserDetails()) {
 				   if(orderDetail.getFood_id().getId() == food.getId()) {
 					   cartForm.getListUserDetails().remove(orderDetail);
	   ...
	   public String shoppingCartUpdateQty(HttpServletRequest request, //
		   ...
		   Order myCart = null;
		      if(username.equals("anonymousUser")) {
		    	  ...
		    	  double total = 0;
	   			  for(OrderDetail orderDetail: myCart.getListUserDetails()) {
	   					total = total + orderDetail.getPrice();
	   			  }
		    	  ...
		    	  for(OrderDetail orderDetail: cartForm.getListUserDetails()) {
		    		  if(orderDetail.getQuantity() == 0) {
		    			  orderDetailService.deleteOrderDetails(orderDetail.getId());
		    			  cartForm.getListUserDetails().remove(orderDetailService);
		    			  double total = 0;
			   			  for(OrderDetail orderDetail2: cartForm.getListUserDetails()) {
			   					total = total + orderDetail2.getPrice();
			   			  }
			   			  cartForm.setTotal_money(total);
			   			  orderService.updateOrder(cartForm);
		    		  }
		    			  ...
		    			  Food food = foodService.getFood(orderDetail.getFood_id().getId());
						  orderDetail.setPrice(food.getPrice() * orderDetail.getQuantity());
		    			  orderDetailService.updateOrderDetail(orderDetail);
		    			  
		    			  ////
		    			  double total = 0;
			   			  for(OrderDetail orderDetail1: cartForm.getListUserDetails()) {
			   					total = total + orderDetail1.getPrice();
			   			  }
						   ...
						   OrderDetail orderDetail = list.get(i).findLineById(food.getId());
			   			   if(orderDetail == null) {
			   				   ...
			   				   list.get(i).getListUserDetails().add(orderDetail);
				   				double total = 0;
				   				for(OrderDetail orderDetail1: list.get(i).getListUserDetails()) {
				   					total = total + orderDetail1.getPrice();
				   				}
			    		...
			    		if(listOrder.get(i).getStatus().equals("Choose")) {
			    			...
			    			if(orderDetail == null) {
				   			...
				   			orderService.updateOrder(listOrder.get(i));
			   				   ...
			   				   orderDetailService.updateOrderDetail(orderDetail);
				   				double total = 0;
				   				for(OrderDetail orderDetail1: listOrder.get(i).getListUserDetails()) {
				   					total = total + orderDetail1.getPrice();
				   				}
				   				System.out.println("tong tien cap nhat: " + total);
